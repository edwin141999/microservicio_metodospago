...
async function createCard(params) {
  ...
  const result = await prisma.tipopago.create({
    data: {
      ...
      numero_tarjeta: bcrypt.hashSync(numero_tarjeta, 10),
      year_expiracion: bcrypt.hashSync(year_expiracion, 10),
      month_expiracion: bcrypt.hashSync(month_expiracion, 10),
      cvv: bcrypt.hashSync(cvv, 10),
      nombre_banco: bcrypt.hashSync(nombre_banco, 10)
    }
    // "user_id": 1,
    // "cvv": "123",
    // "month_expiracion": "5",
